...
#[tokio::test]
async fn test_cast_vote_with_delegate() {
    // Arrange
    let mut governance_test = GovernanceProgramTest::start_new().await;
    let realm_cookie = governance_test.with_realm().await;
    let governed_account_cookie = governance_test.with_governed_account().await;
    ...
    // Act - casting Yes/No vote with a delegate
    ...
    let vote_record_cookie = governance_test
        .with_cast_vote_using_instruction(
            ...
            &token_owner_record_cookie,
            vote,
            // changing the cast vote to use delegate - see GovernanceInstruction::CastVote
            |itx| {
                itx.accounts[5] = AccountMeta::new_readonly(
                    token_owner_record_cookie.governance_delegate.pubkey(),
                    true,
                )
            },
            Some(&[&token_owner_record_cookie.governance_delegate]),
        )
        .await.unwrap();

    // Assert
    // was the delegate set up for the proposal
    let token_owner_record = governance_test
        .get_token_owner_record_account(&token_owner_record_cookie.address)
        .await;
    assert_eq!(
